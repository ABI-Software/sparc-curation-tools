...
class ScaffoldAnnotation(object):
    ...
    def __init__(self, dfRow):
        self._dir = dfRow[FILE_LOCATION_COLUMN]  # This is now redundant.
...
def search_view_files(dataset_dir):
    ...
    # For each result, check if it is likely to be an actual view file.
    # First check is it a small file?
    #   - open file
    #   - translate contents to json
    #   - check json data structure for view info.
    #      - If the json data structure has the following five keys we will say that it is a view file.
    #      - "farPlane", "nearPlane", "upVector", "targetPosition", "eyePosition"
    return [str(x) for x in result]
...
def check_scaffold_view_annotated():
    ...
    # Check if the view is annotated incorrectly.
    # for scaffoldAnnotation in manifestDataFrame.get_real_scaffold_no_view():
    #     errors.append(NoViewError(scaffoldAnnotation.get_location()))
    return errors
